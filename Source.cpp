...
{
	VideoCapture cap1("hungry_awalsir_fore.wmv");// lana liz naomi tyler
	VideoCapture cap("hungry_awalsir_fore.wmv");
Size SizeOfFrame = cv::Size(320, 240);
	...
	for (;;)
	{
		...
		// saving part:
		//std::string savingName = "all_day_mhi" + std::to_string(++cntr) + "_120du.jpg";
	...
	imwrite("hungry_awalsir.bmp", motion);
	imwrite("hungry_awalsir_rotate.bmp", rotate_motion);
